...
echo firmware\image_tool.exe -c Tools\gain_builder\gain_val_image.config -o Tools\gain_builder\m2m_gain_value_image_%VARIANT%.bin -of prog
...
echo Call %ptool% -d winc%CHPFAM% -i Tools\gain_builder\m2m_gain_value_image_%VARIANT%.bin -if prog -w -r -pfw programmer_firmware\release3400\programmer_firmware.bin
