/**
 * @file network_winc3400tls_wrapper.c
 *
 * @brief winc3400 TLS wrapper.
 *
 *
 */
/*
	...
	switch (u8Msg) {
		case SOCKET_MSG_CONNECT:
		{
			...
			if (pstrConnect && pstrConnect->s8Error >= SOCK_ERR_NO_ERROR) {
				...
				/* Check for ALPN negotitation type. */
				switch (alpn_index)
				{
					case 1:
					printf("Negotiated HTTP/2.0.\r\n");
					break;
					case 2:
					printf("Negotiated HTTP/1.1.\r\n");
					break;
					case 3:
					printf("Negotiated Amazon MQTT.\r\n");
					break;
					case 0:
...
int iot_tls_connect(Network *pNetwork, TLSConnectParams params) {
	...
	if(gu32HostIp != 0)
	{
		...
		//setsockopt(tls_socket, SOL_SSL_SOCKET, SO_SSL_SNI, AWS_IOT_MQTT_HOST, sizeof(AWS_IOT_MQTT_HOST));
	
		/* Sets the protocol list to be used for ALPN */
		#if defined(AWS_IOT_ALPN_ENABLED)
		set_alpn_list(tls_socket,"h2 http/1.1 x-amzn-mqtt-ca");
		#endif
